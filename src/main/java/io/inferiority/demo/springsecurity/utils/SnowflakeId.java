...
public class SnowflakeId {
    ...
    /**
     * 这种获取 WorkId 和 DataCenterId的方式是否有可能产生重复id?
     * 可能性极小，但并不是完全没有
     */
    ...
    private static long getDataCenterId() {
        try {
            byte[] bytes = NetworkInterface.getByInetAddress(Inet4Address.getLocalHost()).getHardwareAddress();
            int sum = 0;
            for (byte b : bytes) {
                sum += b;
            }
